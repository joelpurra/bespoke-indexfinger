...
gulp.task('compile', ['clean'], function() {
    return browserify('./lib/bespoke-indexfinger.js')
        .exclude('bespoke-convenient')
        .bundle({
            standalone: 'bespoke.plugins.indexfinger'
        })
        .pipe(source('bespoke-indexfinger.js'))
        .pipe(buffer())
        ...
        .pipe(gulp.dest('dist'))
        .pipe(rename('bespoke-indexfinger.min.js'))
        .pipe(uglify())
        .pipe(header(template([
            '/*! <%= name %> v<%= version %> ',
